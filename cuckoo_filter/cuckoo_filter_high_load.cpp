...
class CuckooFilter {
    private:
        ...
        bool lookup_util(uint64_t fp, uint32_t _hash, size_t bucket_id, size_t count) {
            if (count >= threshold)
                return false;
        ...
        bool remove_util(uint64_t fp, uint32_t _hash, size_t bucket_id, size_t count) {
            if (count >= threshold)
                return false;
    ...
    public:
        explicit CuckooFilter(uint32_t size, size_t relocation_threshold):
            ...
            vector<uint64_t> array(size + 7, 0);
